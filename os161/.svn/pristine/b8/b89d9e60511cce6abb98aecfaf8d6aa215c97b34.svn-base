...
thread_create(const char *name)
{
	...
	// thread->pageTable = init_page_table();


	// If you add things to the thread structure, be sure to initialize
	// them here.

	// Every time a thread starts, it now needs a pid
	// Find first one in the pTable that is open, and use it

	int tableIndex = 0;

	for (tableIndex = 0 ; tableIndex < MAXPROCESS ; tableIndex++){
		if (pTable[tableIndex].pState == P_UNUSED){ //we'll eventually need to find one that's been exited

			pTable[tableIndex].thread = thread;
			pTable[tableIndex].pState = P_RUN;
			pTable[tableIndex].pid = tableIndex;
			pTable[tableIndex].exitSem = sem_create("process_exit_semaphore", 0);//used to make parent wait till i exit when calling waitpid			
			thread->pid = pTable[tableIndex].pid;
			pTable[tableIndex].waitedOn = 0;//used to check if parent has already waited on this to exit
			break;
		}
	}
 ...
 * ADDR.
 the addr may not be the address of the thread, for threads using sempahores, the addr is the addr of the semaphore
 */
void
...
init_proc_table(){
	...
	for (i = 0 ; i < MAXPROCESS ; i++ ){
		pTable[i].thread = NULL;  
		pTable[i].pState = P_UNUSED;
	}

	// we define 'process' as anything created by user
	// i.e. something that goes through sys fork
	// => the first kernel thread should increment this number to 0, so we start it at -1

	numProcess = -1;

	i = 0; // meaningless line just a point to stop when debugging 
}
