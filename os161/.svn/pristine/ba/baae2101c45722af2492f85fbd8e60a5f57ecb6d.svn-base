...
vm_fault(int faulttype, vaddr_t faultaddress)
{
	...
	if (faultaddress >= vbase1 && faultaddress < vtop1) {//text region
		...
		is_executable = 1;
		loadsegmentNeeded = 1;
		offset = faultaddress - vbase1 + as->as_off1;
		...
		is_executable = 0;
		loadsegmentNeeded = 1;
		offset = faultaddress - vbase2 + as->as_off2;
	...
	if(page_in_pagetable(faultaddress)){
		...
		// load_segment will recursivley call vm_fault so create a page before hand
		newEntry = page_create(faultaddress,1);

		if (loadsegmentNeeded){


				// vaddr_t top;
				// size_t filesize;
				// size_t memsize = PAGE_SIZE;
				// if(is_executable){//text region
				// 	// top = as->as_vbase1 + as->as_filesize1;
				// 	filesize = as->as_filesize1 - (faultaddress - as->as_vbase1);
				// }
				// else{//data region
				// 	// top = as->as_vbase2 +  as->as_filesize2;
				// 	filesize = as->as_filesize2 - (faultaddress - as->as_vbase2);
				// }

				// //so i don't error out in load_segment because i'm only trying to load 1 page at a time
				// //happens when not on the last page
				// if(filesize > memsize){
				// 	filesize = memsize;	
				// } 

				// // if( (faultaddress == (top & PAGE_FRAME))  ){//checking if last page
				// // 	filesize = top & ~(PAGE_FRAME);
				// // 	//equivalent to filesize = top - (top & (PAGE_FRAME))
				// // }
				// // else{
				// // 	filesize = PAGE_SIZE;
				// // } 

				// result = load_segment(as->as_v, offset, faultaddress, memsize, filesize, is_executable);
				// if (result){
				// 	return (result);
				// }
				// else{
				// 	return(0);//so i won't write duplicate tlb entries when load_segment cause calls to vm_fault
				// }
	       	}
	}
