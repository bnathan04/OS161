...
load_elf(struct vnode *v, vaddr_t *entrypoint, char* progname)
{
	...
	curthread->t_vmspace->as_progname = kmalloc( (strlen(progname)+ 1) * sizeof(char));
	strcpy(curthread->t_vmspace->as_progname, progname);
