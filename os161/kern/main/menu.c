...
// include syscall functions, to solve synchronization issues
...
cmd_progthread(void *ptr, unsigned long nargs)
{
	...
	result = runprogram(progname, nargs, args, KERNELCALL, 0);
...
common_prog(int nargs, char **args)
{
	...
	struct thread* waitForMe;

	

	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, &waitForMe);
	...
	//wait for the program to end before letting menu write stuff	
	result = sys_waitpid(waitForMe->pid,&status,0, &retVal, NULL);
	if(result){
		return(result);
	}
...
/*
 * Command for setting dbflags variable
args = [dbflags, flagNumber, "on/off"]
sahan, 2017/09/17
*/
static
int
cmd_dbflags(int nargs, char **args)
{
	...
	int numberArg = atoi(args[1]);

	//check valid 2nd arg
	if(numberArg > 12 || numberArg < 1){
		kprintf("Usage: df nr on/off\n");
		return(EINVAL);		
	}
	...
	//get appropriate bit to mask
	u_int32_t flagToChange = 0x1 << (numberArg - 1);


	//on vs off
	if(args[2][1] == 'n'){//on
		
		dbflags = dbflags  | flagToChange;	
	}
	else{//off
		dbflags = dbflags & (~flagToChange);
	}    
...
// Balu, 2017/09/13 - added [dbflags] option
...
// Balu, 2017/09/15/ - dbflags menu & dbflags menu
...
//Edit: Balu, 2017/09/15 - added dbflags command into table
