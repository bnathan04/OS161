...
/* Balu, 2017/09/17 -- new hello world function header*/
#include <hello.h>
...
boot(void)
{
	...
	kprintf("Balu and Sahan's system version %s (%s #%d)\n", 
	...
	// programLock = lock_create("programLock");
...
shutdown(void)
{
	...
	// We addted this
	lock_destroy(forkLock);
	// lock_destroy(programLock);
